...
(function () {
  ...
  function showMatchDetail(match) {
    ...
    var html = '<table class="detail-table" role="table"><thead><tr><th>Namn</th><th>Plan</th><th>Bänk</th><th>%</th><th>Assists</th><th>Fouls</th><th>Mål</th></tr></thead><tbody>';
  ...
  function renderStatsList() {
    ...
    matches.forEach(function (m) {
      ...
      (m.players || []).forEach(function (entry) {
        var id = entry.playerId || entry.playerNameAtTime;
        if (!id) return;
        if (!byPlayer[id]) byPlayer[id] = { totalCourt: 0, totalBench: 0, totalAssists: 0, totalFouls: 0, totalGoals: 0, matchesRecorded: 0, matchesPlayed: 0, courtSum: 0, pctSum: 0 };
        if (!playerNames[id]) playerNames[id] = entry.playerNameAtTime || id;
        var court = entry.secondsOnCourt || 0;
        var bench = Math.max(0, matchSec - court);
        byPlayer[id].totalCourt += court;
        byPlayer[id].totalBench += bench;
        byPlayer[id].totalAssists += entry.assists != null ? entry.assists : 0;
        byPlayer[id].totalFouls += entry.fouls != null ? entry.fouls : 0;
        byPlayer[id].totalGoals += entry.goals != null ? entry.goals : 0;
        byPlayer[id].matchesRecorded += 1;
        if (court > 0) {
          byPlayer[id].matchesPlayed += 1;
          byPlayer[id].courtSum += court;
          byPlayer[id].pctSum += matchSec > 0 ? 100 * court / matchSec : 0;
        }
      });
    });

    var sorted = Object.keys(byPlayer)
      .map(function (id) { return { id: id, name: playerNames[id] || id, data: byPlayer[id] }; })
    ...
    sorted.forEach(function (item) {
      ...
      card.innerHTML =
        ...
        '<div class="stats-grid">' +
          '<span class="stats-label">Total plan</span><span class="stats-value">' + formatMmSs(d.totalCourt) + '</span>' +
          '<span class="stats-label">Total bänk</span><span class="stats-value">' + formatMmSs(d.totalBench) + '</span>' +
          '<span class="stats-label">Matcher registrerade</span><span class="stats-value">' + d.matchesRecorded + '</span>' +
          '<span class="stats-label">Matcher spelade</span><span class="stats-value">' + d.matchesPlayed + '</span>' +
          '<span class="stats-label">Snitt plan/match</span><span class="stats-value">' + formatMmSs(avgCourt) + '</span>' +
          '<span class="stats-label">Snitt % spelad</span><span class="stats-value">' + avgPct + '%</span>' +
          '<span class="stats-label">Assists (total)</span><span class="stats-value">' + (d.totalAssists || 0) + '</span>' +
          '<span class="stats-label">Fouls (total)</span><span class="stats-value">' + (d.totalFouls || 0) + '</span>' +
          '<span class="stats-label">Mål (total)</span><span class="stats-value">' + (d.totalGoals || 0) + '</span>' +
        '</div>';
