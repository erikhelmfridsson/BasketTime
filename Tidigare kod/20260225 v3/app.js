...
(function () {
  ...
  function renderHistoryList() {
    ...
    matches.forEach(function (m) {
      ...
      var subLine = (teamStr ? teamStr + ' · ' : '') + t('history.duration') + ' ' + durStr;
  ...
  function getLineChartData() {
    var matches = loadMatches();
    if (matches.length === 0) return { labels: [], values: [] };
    ...
    sorted.forEach(function (m) {
      ...
      players.forEach(function (p) {
        totalSeconds += (p.secondsOnCourt != null ? p.secondsOnCourt : 0);
      });
      values.push(totalSeconds);
    });
  ...
  function renderStatsLineChart() {
    ...
    if (n === 1) {
      ...
      html += '<circle cx="' + pt[0] + '" cy="' + pt[1] + '" r="5" fill="var(--on-court)" stroke="var(--bg-card)" stroke-width="2"/>';
    } else {
      html += '<polyline fill="none" stroke="var(--on-court)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" points="' + points.join(' ') + '"/>';
    }
    html += '</svg>';
    wrap.innerHTML = html;
  }
