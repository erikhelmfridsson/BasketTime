...
(function () {
  ...
  function renderStatsList() {
    ...
    sorted.forEach(function (item) {
      ...
      var subLine = 'Plan ' + formatMmSs(d.totalCourt) + ' · Bänk ' + formatMmSs(d.totalBench) + ' · ' + d.matchesRecorded + ' matcher · Assists ' + (d.totalAssists || 0) + ' · Fouls ' + (d.totalFouls || 0) + ' · Mål ' + (d.totalGoals || 0);
      var div = document.createElement('div');
      div.className = 'stats-item history-item';
      div.innerHTML =
        '<span class="history-item-date">' + escapeHtml(topLine) + '</span>' +
        '<span class="history-item-duration">' + escapeHtml(subLine) + '</span>' +
        '<table class="detail-table stats-detail-table" role="table">' +
          '<thead><tr><th>Plan</th><th>Bänk</th><th>%</th><th>Matcher</th><th>Assists</th><th>Fouls</th><th>Mål</th><th>Snitt plan</th><th>Snitt %</th></tr></thead>' +
          '<tbody><tr>' +
